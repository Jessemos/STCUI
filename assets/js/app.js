...
//'use strict';
function transpose(dataTotranspose, maxI, maxJ) {
    //Transpose key lead - Agency ID
    let transposedData = new Map();
    {
        for (var j = 0; j < maxJ; j++) {
            mapKey = dataTotranspose[0][j];
            // setVal = new Set(); remove dulicates
            let setVal = [];
            for (var i = 1; i < maxI; i++)
                setVal.push(dataTotranspose[i][j]);
            transposedData.set(mapKey, setVal);
        }
    }
...
function getElementsID() {
    let elementsID = new Map();
    elementsID
        .set("inputID", ["searchByAgencyID", "searchByAreaId", "searchByAgencyUniqueDS", "searchByEditorDSID", "searchByAgencyName", "TBC-searchByAgencyEditorName", "searchByAreaName", "TBC-searchByOwner", "TBC-isWestCoast"])
        .set("suggestionsID", ["agencyIDSuggestions", "areaIdSuggestions", "agencyUniqueDSSuggestions", "DSSuggestions", "agencyNameSuggestions", "TBC", "areaNameSuggestions", "TBC", "TBC"])
        .set("elementTExtValue", ["Agency Id", "Agency's Area ID", "Agencies Data Source Id", "Agency Area Default Data Source", "Agency Name", "Agency SA editor name", "Area Name", "STC OWNER", "West Coast?"]);
...
function getAgencyInfoHtml() {
    ...
    `;
    return agencyAreaInfoHtml;
}

window.addEventListener("DOMContentLoaded", function () {

    function doFirst() {
        let agencyAreaInfoHtml = getAgencyInfoHtml();
        let agencyInfoDiv = [];
    ...
    function agencyInfoautoComplete(elem) {
        let agencyInfo = concatDataMatrix("suggestionsID");
        let dataMatrix = transpose(agencyInfo, agencyInfo.length, agencyInfo[0].length);
        let agencyIndex = dataMatrix.get(elem.attributes["list"].value).indexOf(parseInt(elem.value));
        alert(dataMatrix.get(elem.attributes["list"].value)[agencyIndex]);
        document.getElementById("searchByAgencyName").value = dataMatrix.get("agencyNameSuggestions")[agencyIndex];
    }

    function updateAllSuggestionsList(suggestionsListArray) {
        var finalList = [];
        var optionsLength = 0;
    ...
    function addFormEventListener() {
        ...
        document.addEventListener('focusin', function (e) {
            if (e.target.tagName == "INPUT") {
                {
                    updateSuggestionsList(concatDataMatrix("suggestionsID"));
                }
            }
        }, true);
        ...
        document.addEventListener('blur', function (e) {
            if (e.target.value !== "") {
                alert("Blur value: " + e.target.value);
                agencyInfoautoComplete(e.target);
            }
            else {
